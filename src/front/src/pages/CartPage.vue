<template>
  <div class="cart-page">
    ...
    <div style="margin: 14px;" v-if="address">
      <h2 style="margin: 14px;">Адрес доставки</h2>
      <p>{{ formatAddress(address) }}</p>
    </div>

    <button style="margin: 14px;" @click="placeOrder" v-if="!order">Оформить заказ</button>

    <div style="margin: 14px;" v-if="order">
      <p>Статус: {{ getStatusText(order.status) }}</p>
      <p v-if="order.deliveryCost">Доставка: {{ order.deliveryCost.sum }} {{ getCurrencyText(order.deliveryCost.currency) }}</p>
      <button style="margin: 14px;" v-if="order.status === 0" @click="payOrder">Оплатить заказ</button>
      <p v-if="order.status === 1">Товар успешно оплачен и отправлен на сборку</p>
    </div>
  </div>
</template>
